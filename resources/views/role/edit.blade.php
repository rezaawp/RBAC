@php
    $roleName = isset($role) ? $role?->name : '-';
@endphp
@extends('panel.layout.settings')
@section('title', __('Manage ' . $roleName . ' Permissions'))
...
@section('settings')
    ...
    <p class="mb-8">
        @lang('Has full control over the entire system, including the ability to manage all roles and change all settings without restrictions.')
    </p>
    <p class="mb-8">
    ...
    <form
            ...
            action="{{ route('dashboard.admin.rbac.roles.permissions.save') }}"
            method="POST"
    >
        @csrf
